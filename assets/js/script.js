...
class Pet {
    ...
    static sameOwner(p1,p2) {
        return  p1.ownerName.toLowerCase() === p2.ownerName.toLowerCase() ? true : false
      }
}
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    document.querySelector('form').addEventListener('submit', (e) => {
        ...
        for (const pet of pets) {
            tableBody.insertAdjacentHTML('beforeend', `<tr><td>${pet.petName}</td><td>${pet.ownerName}</td><td>${pet.species}</td><td>${pet.breed}</td></tr>`);
        }
    });
})
